...
class SpiderSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        for movie in movies_array:
            title = movie.xpath('./div/@title').extract_first()
            tag = movie.xpath('./div[2]/text()[2]').extract_first().replace('\n', '').strip()
            release_time = movie.xpath('./div[4]/text()[2]').extract_first().replace('\n', '').strip()
